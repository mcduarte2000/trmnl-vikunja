...
- Filters are applied before sorting and limiting. The result is sorted by due date ascending (soonest first) then priority descending in Task View (Simple List), or by priority descending then due date ascending in Kanban View, then capped to `tasks_per_view`.
...
The result is then sorted — by due date ascending (soonest first) then priority descending in Task View, or by priority descending then due date ascending in Kanban View — and limited to `6` tasks.
